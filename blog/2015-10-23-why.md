...
In a coupe of days Ceylon 1.2 will be released, after a year of development. That’s exciting for us, 
...
Finally, Ceylon's module system is already compatible with OSGi, with Vert.x, with `npm`, and with 
`requirejs`. When Jigsaw finally arrives, we'll support it too.
...
their own static type system. A handful of languages such as ML and Haskell eschew this, offering a 
truly a principled, disciplined approach to static typing. And programmers working with these 
languages report a curious thing: that their programs have a strange tendency to work first or 
second time they run them. Sure, it takes a little longer to produce a program that the compiler 
accepts, but once the compiler is satisfied, so many common bugs have already been eliminated, that 
the program is often already correct or at least _almost_ correct.
...
Ceylon is evolving into a truly compelling platform for web development, allowing you to reuse
code written for a Java server on the JavaScript client and vice-versa, while still interoperating 
