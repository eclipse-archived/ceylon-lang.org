...
with Java 8 lambdas and [streams][], and with [Spring Boot][].
This release also introduces support for [`static` members].
...
Compared to previous releases of Ceylon, the use of 
annotation-driven Java frameworks based on reflection is now
much more transparent, and integration of large multi-module 
Maven-based libraries and frameworks is now much simpler.

The Ceylon tour has been extensively updated, especially the
chapters dealing with interoperation with native 
...
- new Maven interop mode `--fully-export-maven-dependencies`
  ...
  Spring Boot
- support for POM-only artifacts in Maven interop
- new [Java EE-friendly compiler mode][EE mode] making it
  easy to use Java frameworks that depend on reflective
  direct access to fields
- ability to pass Ceylon metamodel to Java APIs accepting 
  `java.lang.Class`
- ability to pass Ceylon Strings to Java APIs accepting 
...
- several bugfixes to relating to interop with overloaded 
  Java methods
- several new compiler options allowing for incremental
  compilation and compilation of dependencies
- compiler option for setting JVM options (no more need
  for `JAVA_OPTS`)
...
and `ceylon.interop.persistence` were introduced.
